...
export async function CreateSubscriptionNFT(userAddr, image, amount) {
  ...
  //     data: window.contract.methods.batchMint(userAddr, amount).encodeABI(), //make call to NFT smart contract 
  // };
  //set up your Ethereum transaction
  const transactionParameters = {
    to: subscriptionContractAddr, // Required except during contract publications.
    from: userAddr, // must match user's active address.
    data: window.contract.methods.batchMint(userAddr, amount, metaJson).encodeABI(), //make call to NFT smart contract   
  };
...
export async function getMySubscriptionNFT(userAddr) {
  const web3 = new Web3(new Web3.providers.HttpProvider(process.env.REACT_APP_ETHEREUM_RPC_URL));
  let token_id
  ...
  for (let i = 0; i < myBalance; i++) {    
    tokenId = await contract.methods.tokenOfOwnerByIndex(userAddr, i).call()    
...
export async function SubscribeUser(userAddr, myAddr, setSubscriptionsCnt) {
  ...
  for (let i = 0; i < myBalance; i++) {
    ...
    const ownerAddr = await subscriptionContract.methods.ownerOf(tokenId).call();
    if (ownerAddr === userAddr) {
  ...
  salefactoryContract.events.NewSale({})
    ...
    //     return          
    //   }
    //   console.log('응답: ', receipt)
    //   });
  // address : "0x893c964e43759f1a02993889194c8566e5f5d4f5"
  // topic[0] : 0x874bdb649427dfeceaa7f10beee59a823d3e45a6814f50bc18ba2adefd058e48
  // let saleContractAddrList = await salefactoryContract.methods.allSales().call();
  // let saleContractAddr = saleContractAddrList[saleContractAddrList.length - 1];
  // console.log('사용할 Sale 컨트랙트 주소: ', saleContractAddr)
